...
shadowsocks_pass="$(echo $(date +%s) $(dd if=/dev/urandom bs=100 count=1) | sha256sum | head -c $((26 + RANDOM % 32)))"
...
echo "$SSURI" | sudo tee /opt/shadowsocks/client-url_$shadowsocks_port.sh
sudo chown ec2-user:root /opt/shadowsocks/client-url_$shadowsocks_port.sh
sudo chmod u=rx,g=,o= /opt/shadowsocks/client-url_$shadowsocks_port.sh
